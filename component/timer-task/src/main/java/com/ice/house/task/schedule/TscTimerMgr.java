...
public class TscTimerMgr extends Actor {
    ...
    public void init() {
        for (int c = 0; c < this.TICKS; ++c) {
            this.wheel.add(new HashSet<TscTimer>()); /* 初始化线程上的时间轮. */
        }
    ...
    public void quartz(long now) {
        if (now - this.lts < ODateu.SECOND) /* 不足一秒. */ {
            return;
        }
        //
        this.lts = now;
        this.loop(); /* 执行定时器检查. */
        this.slot += 1; /* 指针跳动. */
        this.slot = this.slot == this.TICKS ? 0 : this.slot;
    }
    ...
    public void cancelTimer(TscTimer timer) {
        this.future(v -> this.cancel(timer));
    }
